...
const App: React.FC = () => {
  ...
  const handleSavePlace = async (placeData: Partial<Place>) => {
    ...
    setIsLoading(true);
    try {
      // Pastikan data dipetakan dengan benar ke snake_case database
      const payload = {
        name: placeData.name,
        category: placeData.category,
        address: placeData.address,
        description: placeData.description,
        reference_url: placeData.referenceUrl || placeData.reference_url,
        place_photo_url: placeData.placePhotoUrl || placeData.place_photo_url,
        menu_photo_url: placeData.menuPhotoUrl || placeData.menu_photo_url,
  ...
  const filteredPlaces = useMemo(() => {
    return places.filter(place => {
      ...
      const addrMatch = place.address?.toLowerCase().includes(searchQuery.toLowerCase());
      const tagMatch = place.tags?.some(t => t.toLowerCase().includes(searchQuery.toLowerCase()));
      
      const matchesSearch = nameMatch || addrMatch || tagMatch;
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                  {supabase ? 'Supabase Connected' : 'Supabase Not Found'}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                   <p className="text-xs font-medium text-slate-700 leading-relaxed">
                     ...
                     3. Masukkan Project URL dan Anon Key ke variabel di <b>App.tsx</b> baris 12 & 13.
                   </p>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl overflow-y-auto" onClick={() => setViewingPlace(null)}>
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl my-auto animate-modal-in overflow-hidden flex flex-col lg:flex-row max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="w-full lg:w-1/2 bg-slate-100 overflow-y-auto no-scrollbar">
              {(viewingPlace.place_photo_url || viewingPlace.placePhotoUrl) && <div className="p-4"><img src={viewingPlace.place_photo_url || viewingPlace.placePhotoUrl} className="w-full rounded-[2rem] shadow-lg mb-6" alt="Foto" /></div>}
              {(viewingPlace.menu_photo_url || viewingPlace.menuPhotoUrl) && <div className="p-4 pt-0"><p className="text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest px-4">Foto Menu</p><img src={viewingPlace.menu_photo_url || viewingPlace.menuPhotoUrl} className="w-full rounded-[2rem] shadow-lg" alt="Menu" /></div>}
            </div>
            <div className="w-full lg:w-1/2 p-10 lg:p-14 overflow-y-auto flex flex-col">
              ...
              <div className="flex gap-4">
                <a href={viewingPlace.reference_url || viewingPlace.referenceUrl} target="_blank" className="flex-grow py-5 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl text-center shadow-2xl hover:bg-indigo-700 transition-all">Buka Google Maps</a>
